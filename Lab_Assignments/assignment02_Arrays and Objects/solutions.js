...
function formatNames() {
  ...
  for (let i = 0; i < users.length; i++) {
    var name = users[i].name.first + ' ' + users[i].name.last;
    console.log(name);
  }
}
...
userUtils.getOldestUser = function() {
  ...
  users.forEach(function(element) {
    userAges.push(getAgeInYears(new Date(element.birthDate)));
  });

  // for(const element of users){
  //   let temp = getAgeInYears(new Date(element.birthDate));
  //   userAges.push(temp);
  // }
  ...
  for (var i = 1; i < userAges.length; i++) {
    if (userAges[i] > max) {
      maxIndex = i;
      max = userAges[i];
    }
...
userUtils.getProfiles = function(width, height) {
  ...
  // email: 'Paige Bools <pbools0@webmd.com>',
  // image: 'https://faces.com/user/1?width=150&height=150'
  var profiles = users.map(function(element) {
    if (height === undefined) {
      height = width;
    }
...
userUtils.updateAccessCount = function(amount, ids) {
  ...
  var updated = [];
  var inputIdx = [];
  inputIdx = inputIdx.concat(ids);

  for (const elem of inputIdx) {
    var findVal = users.find(function(element) {
      if (element.id === elem) {
        element.accessCount += amount;
        return element;
      }
    });
    ...
    if (findVal !== undefined) {
      var info = {
        id: elem,
        accessCount: users[elem - 1].accessCount
      };
      updated.push(info);
    }
  }
...
userUtils.search.byName = function(name, fuzzy) {
  ...
  var result = users.filter(function(element) {
    ...
    if (fuzzy === true) {
      ...
      if (patt.test(element.name.first) || patt.test(element.name.last)) {
        bool = true;
      }
    } else {
...
userUtils.search.byCountry = function(...countries) {
  ...
  for (const country of countries) {
    var result = users.filter(function(element) {
      if (element.address.country === country) {
        return element;
      }
    });
...
userUtils.createWelcomeEmails = function() {
  // TODO...
  var noAccessUsers = users.filter(function(element) {
  ...
  var emailContents = {};
  noAccessUsers.forEach(function(element) {
    let key = element.contact.email;
    emailContents[key] =
      //`...text of email message ${element.name.first} ${element.name.last}...`;
      `Dear ${element.name.first} ${element.name.last}
Welcome to imaginary application!  We're so happy you joined, and wanted
